...
const CanceledTodo = () => {
    useEffect(() => {
        TodoListByStatus("Canceled");
    }, []);
    
    const canceledPlans = useSelector((state) => state.todos.Canceled);
    ...
    return (
        <div className="pr-10">
            <h1 className="text-3xl text-center text-black font-roboto font-bold my-10">Canceled todo</h1>

            {canceledPlans.length > 0 &&
                <div className="flex justify-between gap-y-8 flex-wrap">
                    {canceledPlans?.map((item, i) => (
