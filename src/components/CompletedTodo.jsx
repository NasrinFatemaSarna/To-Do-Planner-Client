...
const CompletedTodo = () => {
    useEffect(() => {
        TodoListByStatus("Completed");
    }, []);
    
    const completedPlans = useSelector((state) => state.todos.Completed)
    ...
    return (
        <div className="pr-10">
        <h1 className="text-3xl text-center text-black font-roboto font-bold my-10">Completed todo</h1>

        {completedPlans.length > 0 &&
            <div className="flex justify-between gap-y-8 flex-wrap">
                {completedPlans?.map((item, i) => (
